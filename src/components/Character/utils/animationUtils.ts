  ...
  if (gltf.animations) {
    ...
    clipNames.forEach((name) => {
      ...
      if (clip) {
        ...
        action!.play();
        action!.timeScale = 0.95;  // Slightly slower, gentler idle motions
      } else {
    ...
    if (typingAction) {
      ...
      typingAction.play();
      typingAction.timeScale = 0.9;  // Gentler typing gestures
    }
  }
  function startIntro() {
    const introClip = gltf.animations.find(
    ...
    setTimeout(() => {
      ...
      mixer.clipAction(blink!).play().fadeIn(0.7);  // Softer blink
    }, 2500);
  }
  function hover(gltf: GLTF, hoverDiv: HTMLDivElement) {
    ...
    const onHoverFace = () => {
      if (eyeBrowUpAction && !isHovering) {
        ...
        eyeBrowUpAction.enabled = true;
        eyeBrowUpAction.setEffectiveWeight(1.8);  // Subtler brow raise
        eyeBrowUpAction.fadeIn(0.7).play();      // Softer, slower raise
      }
    };
    const onLeaveFace = () => {
      if (eyeBrowUpAction && isHovering) {
        isHovering = false;
        eyeBrowUpAction.fadeOut(0.8);  // Gentler fade back
      }
    };
    if (!hoverDiv) return;
